...
// Interface for array item data
interface ConfiguratorColorItem {
  _key: string
  name?: string
  colorSlug?: {current: string}
...
export default defineType({
  ...
  fields: [
    defineField({
      ...
      validation: (Rule) =>
        Rule.required().integer().positive().error('Frame count must be a positive whole number.'),
    }),
    defineField({
      ...
      of: [
        defineArrayMember({
          ...
          preview: {
            ...
            prepare({name, slug, color, isDefault}) {
              return {
                title: `${name || 'Unnamed Color'} ${isDefault ? '(Default)' : ''}`,
                subtitle: `Slug: ${slug || 'N/A'} | Swatch: ${color || 'N/A'}`,
                media: DropIcon, // Use standard icon instead of custom React component
              }
            },
          },
        }),
      ...
      validation: (Rule) => [
        ...
        Rule.custom((colors: ConfiguratorColorItem[] | undefined) => {
